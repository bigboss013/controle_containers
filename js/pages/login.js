...
async function testConnection() {
  ...
  try {
    const doc = await FirestoreRest.getDoc('usuarios', 'admin');
    if (doc) {
      errEl.innerHTML = `✅ Firestore OK - admin: ${JSON.stringify(doc)}<br><small>Login deve funcionar.</small>`;
    } else {
      errEl.innerHTML = '⚠️ Admin não encontrado no Firestore';
    }
  } catch (e) {
    if (e.message.includes('429') || e.message.includes('Servidor ocupado')) {
      errEl.innerHTML = '❌ Rate limit ativo (429). Aguarde alguns minutos e tente novamente.<br><small>O limite reseta ~3h BRT.</small>';
    } else {
      errEl.innerHTML = `❌ Erro: ${e.message}`;
    }
  }
}
